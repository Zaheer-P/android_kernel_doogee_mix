...
write_cmos_sensor(0x4800,0x20);    //mipi��?D?��?��?D?  20��?��?D?   00��?D?
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			...
			LOG_INF("Read sensor id fail, i2c write id: 0x%x, sensor id: 0x%x (Read fail!)\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
            LOG_INF("SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",(MUINT32)(*feature_data),(MUINT32)(*(feature_data+1)),(MUINT32)(*(feature_data+2)));
